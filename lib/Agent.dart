...
class _MyHomePageState extends State<MyHomePage> {
  ...
  String? status, cardNum, reason, pin,amount;
  void initState() {
    _tagRead();
  }
  ...
  Future<void> _handleButtonPress() async {
    ...
    if (((reason?.isEmpty ?? true) ||
      ...
      try {
        final data =
        await makeTransaction(double.parse(amount!), cardNum!, pin!, status!);
        if (data == true) {
          ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(content: Text("Transaction done!")));
        } else {
          ScaffoldMessenger.of(context).showSnackBar( SnackBar(
              content: Text('Transaction failed try again! CardNum:'+cardNum.toString())));
        }
